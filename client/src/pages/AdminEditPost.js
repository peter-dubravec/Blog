...
export default function CreatePost() {
    ...
    useEffect(() => {
        const fetchArticle = async () => {
            const response = await fetch(`http://localhost:5000/api/admin/dashboard/${id}/without-comments`, {
                headers: {
    ...
    const log = async () => {
        if (editorRef.current) {
            const response = await fetch(`http://localhost:5000/api/admin/dashboard/${id}/update`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${user.token}`,
