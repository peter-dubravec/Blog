...
const AdminGetPost = () => {
    ...
    useEffect(() => {
        const fetchArticle = async () => {
            const response = await fetch(`http://35.180.74.202:5000/api/admin/dashboard/${id}`, {
                headers: {
    ...
    const deletePost = async (id) => {
        const response = await fetch(`http://35.180.74.202:5000/api/admin/dashboard/${id}/delete`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${user.token}`
            }
    ...
    const handlePublish = async (id, action) => {
        const response = await fetch(`http://35.180.74.202:5000/api/admin/dashboard/${id}/publish`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${user.token}`,
    ...
    const handleDeleteComment = async (id) => {
        const response = await fetch(`http://35.180.74.202:5000/api/admin/dashboard/comment/${id}/delete`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${user.token}`
            }
